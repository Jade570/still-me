...
import { setOverlay } from "../ui/view";
...
export async function initApp() {
  ...
  const screens = await createScreens({
    ...
    // screen modules read engine through this provider
    getEngine: () => engine,
  });
  ...
  dom.startControlButton.addEventListener("click", () => {
    setOverlay(dom, "controls");
    screens.show(1);
    console.log(env.JOIN_LINK_TEXT);
